...
const Page = () => {
  const { data } = useData();
  const lastDate = data?.events.sort((evtA, evtB) =>
    new Date(evtA.date) < new Date(evtB.date) ? -1 : 1
  );
  const lastObject = lastDate && lastDate[0];
  return (
    <>
      <header>
        <Menu />
      ...
      <footer className="row">
        <div className="col presta">
          ...
          <EventCard
            imageSrc={lastObject?.cover}
            imageAlt={lastObject?.title}
            title={lastObject?.title}
            date={new Date(lastObject?.date)}
            small
            label="boom"
          />
        </div>
